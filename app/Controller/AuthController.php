...
class AuthController {
    ...
    public function resendVerificationOtp() {
        ...
        if (isset($_SESSION['otp_cooldown_timestamp']) && time() - $_SESSION['otp_cooldown_timestamp'] < 60) {
            http_response_code(429);
            echo json_encode(['success' => false, 'message' => 'Vui lòng chờ 60 giây trước khi yêu cầu gửi lại mã.']);
            exit;
        }
        ...
        try {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Không thể gửi OTP do lỗi hệ thống (Cấu hình SMTP chưa đúng hoặc sự cố mạng).',
                'error' => $e->getMessage()
            ]);
            exit;
        }

        $_SESSION['otp_cooldown_timestamp'] = time();
        http_response_code(200);
    ...
    public function changePassword() {
        ...
        // if user registered via google and has no password, they can't use this route properly unless we add a flow for them
        if ($user['password'] && !password_verify($currentPassword, $user['password'])) {
    ...
    public function googleLoginCallback() {
        ...
        // Disable aud check locally if no client ID is set for development
        if ($googleClientId && isset($payload['aud']) && $payload['aud'] !== $googleClientId) {
