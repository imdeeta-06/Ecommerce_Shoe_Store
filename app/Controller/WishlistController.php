...
class WishlistController {
    ...
    public function add() {
        ...
        if ($productId > 0) {
            ...
            $stmt = $db->prepare("SELECT p.id FROM product p LEFT JOIN categories c ON c.id = p.category_id WHERE p.id = :id AND (p.status = 1 OR p.status = '1' OR p.status = 'active') AND (p.category_id IS NULL OR c.status = 1 OR c.status = '1' OR c.status = 'active') LIMIT 1");
