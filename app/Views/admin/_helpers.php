...
if (!function_exists('adminStart')) {
    function adminStart($title, $active, $flash = null) {
        ...
        $adminAvatar = !empty($_SESSION['user_avatar']) ? BASE_URL . $_SESSION['user_avatar'] : 'https://ui-avatars.com/api/?name='.urlencode($adminName).'&length=1&background=111&color=fff&size=40';
        ?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <meta name="csrf-token" content="<?= adminE(\App\Helpers\SessionHelper::csrfToken()) ?>">
    ...
    <aside class="admin-sidebar" id="adminSidebar" aria-label="Menu quản trị">
        ...
        <nav class="admin-nav">
            <ul>
                ...
                <li><a href="<?= BASE_URL ?>admin/procurement" class="<?= $active === 'procurement' ? 'active' : '' ?>">Mua hàng & công nợ</a></li>
