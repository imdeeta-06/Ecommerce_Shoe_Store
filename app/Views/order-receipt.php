...
foreach (($order['items'] ?? []) as $item) {
    $taxRateLabels[] = ($item['tax_category_snapshot'] ?? '') === 'not_subject'
        ? 'Không chịu thuế'
        : number_format((float)($item['tax_rate_snapshot'] ?? 0), 2, ',', '.') . '%';
}
if ((float)($order['shipping_fee'] ?? 0) > 0) {
    $taxRateLabels[] = ($order['shipping_tax_category'] ?? '') === 'not_subject'
        ? 'Không chịu thuế'
        : number_format((float)($order['shipping_tax_rate'] ?? 0), 2, ',', '.') . '%';
}
$taxRateLabels = array_values(array_unique($taxRateLabels));
?>
<!doctype html>
<html lang="vi">
<head>
...
<main class="sheet">
    ...
    <h1>HÓA ĐƠN BÁN HÀNG — BILL VAT (MÔ PHỎNG)</h1>
    ...
    <table>
        <thead><tr><th>STT</th><th>Sản phẩm</th><th>Phân loại</th><th class="number">SL</th><th class="number">Đơn giá đã thuế</th><th class="number">Thuế suất</th><th class="number">Tiền thuế</th><th class="number">Thành tiền</th></tr></thead>
        <tbody><?php foreach (($order['items'] ?? []) as $index => $item): ?><tr><td><?= $index + 1 ?></td><td><?= $e($item['product_name_snapshot'] ?? $item['product_name']) ?></td><td><?= $e(trim(($item['size'] ?? '') . ' ' . ($item['color'] ?? ''))) ?></td><td class="number"><?= (int)$item['quantity'] ?></td><td class="number"><?= $money($item['price_at_time']) ?></td><td class="number"><?= ($item['tax_category_snapshot'] ?? '') === 'not_subject' ? 'KCT' : $e(number_format((float)($item['tax_rate_snapshot'] ?? 0), 2, ',', '.') . '%') ?></td><td class="number"><?= $money($item['tax_amount'] ?? 0) ?></td><td class="number"><?= $money((float)$item['price_at_time'] * (int)$item['quantity'] - (float)($item['discount_amount'] ?? 0)) ?></td></tr><?php endforeach; ?></tbody>
    </table>
    <table class="totals">
        ...
        <tr><td>Tiền trước thuế</td><td class="number"><?= $money($order['taxable_amount'] ?? 0) ?></td></tr>
        <?php if ((float)($order['non_taxable_amount'] ?? 0) > 0): ?><tr><td>Tiền không chịu thuế</td><td class="number"><?= $money($order['non_taxable_amount']) ?></td></tr><?php endif; ?>
        <tr><td>Thuế suất VAT (%)</td><td class="number"><?= $e(implode(', ', $taxRateLabels) ?: 'Không có') ?></td></tr>
        <tr><td>Tổng tiền thuế VAT (đã gồm trong giá)</td><td class="number"><?= $money($order['tax_amount'] ?? 0) ?></td></tr>
        <tr class="total"><td>Tổng tiền khách phải trả</td><td class="number"><?= $money($order['final_amount']) ?></td></tr>
    </table>
