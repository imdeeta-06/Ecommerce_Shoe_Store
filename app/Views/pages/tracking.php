...
<main class="client-page">
    ...
    <div class="client-main-content" style="max-width: 600px; margin: 0 auto;">
        <p style="text-align: center; margin-bottom: 2rem; color: #666;">Nhập số điện thoại hoặc Gmail nhận hàng để kiểm tra tình trạng đơn hàng của bạn.</p>
        ...
        <form action="<?= BASE_URL ?>tracking" method="GET" style="border: 1px solid #eee; padding: 2rem; background: #fff;">
            <div class="client-form-group" style="margin-bottom: 2rem;">
                <label class="client-label">Số điện thoại hoặc Gmail</label>
                <input type="text" name="contact" class="client-input" placeholder="0901234567 hoặc email@gmail.com" value="<?= htmlspecialchars($contact ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
            </div>
        ...
        <?php elseif (!empty($trackingResults)): ?>
            <?php
            ...
            ?>
            <?php foreach ($trackingResults as $trackingResult): ?>
                <section style="margin-top: 1.5rem; border: 1px solid #e5e7eb; background: #fff; padding: 1.5rem; border-radius: 8px;">
                    <div style="display:flex;justify-content:space-between;gap:1rem;flex-wrap:wrap;border-bottom:1px solid #eee;padding-bottom:1rem;margin-bottom:1rem;">
                        ...
                        <span style="font-weight:600; color: var(--primary-dark);">Trạng thái: <?= htmlspecialchars($trackingStatusLabels[$trackingResult['status']] ?? $trackingResult['status'], ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;line-height:1.7; font-size:0.95rem;">
                        <div><strong>Người nhận:</strong><br><?= htmlspecialchars($trackingResult['shipping_name'] ?? '', ENT_QUOTES, 'UTF-8') ?></div>
                        <div><strong>Tổng số tiền:</strong><br><strong style="color:#c94a4a;"><?= number_format((float)($trackingResult['final_amount'] ?? 0), 0, ',', '.') ?> ₫</strong></div>
