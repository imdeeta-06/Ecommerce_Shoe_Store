...
<?php
// --- BUNDLE RECOMMENDATION ENGINE (Combo Hoan Hỷ) ---
$db_bundle = \App\Models\Database::getInstance()->getConnection();
$bundle_product = null;
if (isset($product['id'])) {
    $current_id = (int)$product['id'];
    $current_category = (int)$product['category_id'];
    
    // Find a top-selling product from a different category to bundle
    $stmt_bundle = $db_bundle->prepare("
        SELECT p.id, p.name, p.base_price, c.name as category_name,
               (SELECT image_url FROM product_images pi WHERE pi.product_id = p.id ORDER BY pi.is_primary DESC, pi.id ASC LIMIT 1) as image_url
        FROM product p
        JOIN categories c ON c.id = p.category_id
        WHERE p.id != :current_id 
          AND p.category_id != :current_category 
          AND p.status = 1 
          AND c.status = 1
        ORDER BY p.sold_count DESC, p.id DESC
        LIMIT 1
    ");
    $stmt_bundle->execute(['current_id' => $current_id, 'current_category' => $current_category]);
    $bundle_product = $stmt_bundle->fetch(PDO::FETCH_ASSOC);
}
...
<div class="product-detail-container">
    ...
    <div class="pd-grid">
        ...
        <div class="pd-info-box">
            ...
            <!-- Combo Hoan Hỷ Widget (Cross-selling) -->
            <?php if ($bundle_product): ?>
                <?php 
                $main_price = (float)$basePrice;
                $rec_price = (float)$bundle_product['base_price'];
                $combo_original = $main_price + $rec_price;
                $combo_discounted = round($combo_original * 0.95); // 5% discount
                $main_img = !empty($imagesList) ? productDetailAssetPath($imagesList[0]['image_url']) : '';
                $rec_img = productDetailAssetPath($bundle_product['image_url'] ?? '');
                ?>
                <div class="lh-combo-box" style="margin: 2rem 0; padding: 1.5rem; background: #fff; border: 2px dashed #b8976b; border-radius: 16px; box-shadow: 0 4px 15px rgba(184, 151, 107, 0.08);">
                    <h4 style="font-family: var(--font-ui); color: #3e2723; font-weight: 700; margin: 0 0 1rem; font-size: 1.05rem; display: flex; align-items: center; gap: 6px;">
                        🌸 Combo Hoan Hỷ (Mua kèm giảm ngay 5%)
                    </h4>
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 1.2rem; flex-wrap: wrap;">
                        <!-- Main Product -->
                        <div style="display: flex; align-items: center; gap: 10px; flex: 1; min-width: 140px;">
                            <img src="<?= BASE_URL . htmlspecialchars($main_img) ?>" style="width: 60px; height: 60px; object-fit: contain; background: #f8f9fa; border-radius: 8px; border: 1px solid #ebdcc6;">
                            <div>
                                <div style="font-size: 0.82rem; font-weight: 600; color: #333; max-width: 120px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;"><?= htmlspecialchars($product['name']) ?></div>
                                <span style="font-size: 0.85rem; font-weight: 700; color: #111;"><?= number_format($main_price, 0, ',', '.') ?> ₫</span>
                            </div>
                        </div>

                        <!-- Plus Sign -->
                        <div style="font-size: 1.5rem; font-weight: 700; color: #b8976b; user-select: none;">+</div>

                        <!-- Recommended Product -->
                        <div style="display: flex; align-items: center; gap: 10px; flex: 1; min-width: 140px;">
                            <img src="<?= BASE_URL . htmlspecialchars($rec_img) ?>" style="width: 60px; height: 60px; object-fit: contain; background: #f8f9fa; border-radius: 8px; border: 1px solid #ebdcc6;">
                            <div>
                                <div style="font-size: 0.82rem; font-weight: 600; color: #333; max-width: 120px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;"><?= htmlspecialchars($bundle_product['name']) ?></div>
                                <span style="font-size: 0.85rem; font-weight: 700; color: #111;"><?= number_format($rec_price, 0, ',', '.') ?> ₫</span>
                            </div>
                        </div>
                    </div>

                    <div style="border-top: 1px dashed #ebdcc6; padding-top: 1rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        <div>
                            <div style="font-size: 0.8rem; color: #666;">Giá trọn bộ Combo:</div>
                            <div style="display: flex; align-items: baseline; gap: 8px;">
                                <span style="font-size: 1.25rem; font-weight: 800; color: #c94a4a;"><?= number_format($combo_discounted, 0, ',', '.') ?> ₫</span>
                                <del style="font-size: 0.85rem; color: #999;"><?= number_format($combo_original, 0, ',', '.') ?> ₫</del>
                            </div>
                        </div>
                        <button type="button" onclick="addBundleToCart(<?= $product['id'] ?>, <?= $bundle_product['id'] ?>)" class="btn-add-cart-main" style="max-width: 130px; font-size: 0.88rem; padding: 10px 16px; border-radius: 50px; background: #3e2723;">
                            Mua Trọn Bộ
                        </button>
                    </div>
                </div>
                        ...
                        <div class="pd-review-card">
                            <div class="pd-review-head">
                                <div class="pd-reviewer-info">
                                    ...
                                    <div>
                                        <div class="pd-reviewer-name" style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                            <?= htmlspecialchars($rev['full_name'] ?? $rev['display_name'] ?? $rev['user_name'] ?? 'Phật tử Liên Hoa') ?>
                                            <?php if (!empty($rev['order_id']) || !empty($rev['order_item_id'])): ?>
                                                <span style="font-size: 0.7rem; background: #e8f5e9; color: #2e7d32; border: 1px solid #c8e6c9; border-radius: 4px; padding: 1px 5px; font-weight: 700; display: inline-flex; align-items: center; gap: 3px;">
                                                    ✓ Đã mua hàng
                                                </span>
                                            <?php endif; ?>
                                        </div>
...
function addBundleToCart(mainId, bundleId) {
    const formData1 = new FormData();
    formData1.append('product_id', mainId);
    formData1.append('qty', 1);
    
    const activeSizeEl = document.querySelector('.pd-size-item.active');
    if (activeSizeEl) {
        formData1.append('variant_id', activeSizeEl.dataset.variantId || '0');
        formData1.append('size', activeSizeEl.dataset.size || '');
    }
    formData1.append('color', selectedColor || 'Tiêu chuẩn');

    showToast('Đang thỉnh Combo...');
    ...
    .then(data => {
        if (data.success) {
            ...
            formData2.append('product_id', bundleId);
            formData2.append('qty', 1);
            formData2.append('color', 'Tiêu chuẩn');
            ...
            throw new Error(data.message || 'Lỗi thêm sản phẩm chính');
        }
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            showToast('Đã thêm trọn bộ Combo Hoan Hỷ!');
            if (typeof window.updateBadgeGlobal === 'function') {
                window.updateBadgeGlobal(data.cart_count);
            }
            setTimeout(() => {
            ...
            showToast(data.message || 'Lỗi thêm sản phẩm phụ');
        }
    })
    .catch(err => {
        console.error(err);
        showToast('Có lỗi xảy ra khi thêm combo.');
    });
}
